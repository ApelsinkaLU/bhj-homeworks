class Autocomplete {
  ...
  getMatches(text) {
    const input = document.querySelector(".autocomplete__search"),
          select = [...document.querySelector(".autocomplete__input")];

    const arr = [];
    select.forEach(el => {
      text = input.value;
      if (el.textContent.includes(text) === true) {
        arr.push({ text: el.textContent, value: el.value });
      }
    });
    return arr;
  }
